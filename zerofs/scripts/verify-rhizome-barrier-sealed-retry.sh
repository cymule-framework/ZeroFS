...
python3 - "$RETRY_ROOT/attempt.receipt" "$RUN_ID" "$$" "$FINALIZER_START" "$FINALIZER_BOOT" \
    "$FINALIZER_CGROUP" "$EVIDENCE_ROOT_DEVICE_INODE" \
    "$(field sealed_retry_verifier_sha256)" "$(field sealed_retry_verifier_device_inode_size)" \
    "$SOURCE_FINAL_SEAL_SHA" "$ATTEMPTED_AT" <<'PY'
import os,sys
path,run,pid,start,boot,cgroup,evidence,digest,identity,source_seal,when=sys.argv[1:]
...
sync -f "$RETRY_ROOT"
python3 - "$RETRY_ROOT/FINAL.receipt" "$RUN_ID" "$EVIDENCE_ROOT_DEVICE_INODE" \
    "$RETRY_MANIFEST_SHA" "$RETRY_RECEIPT_SHA" "$SOURCE_FINAL_SEAL_SHA" "$STATUS_PASS_SHA" \
    "$(field sealed_retry_verifier_sha256)" <<'PY'
import sys
path,run,evidence,manifest,retry,source,status,verifier=sys.argv[1:]
expected={'schema','run_id','verdict','evidence_root_device_inode','retry_manifest_sha256',
          'retry_manifest_readback','retry_receipt_sha256','source_final_seal_sha256',
          'status_pass_receipt_sha256','verifier_sha256'}
values={}; raw=open(path,'rb').read(); text=raw.decode(); assert text.endswith('\n')
for line in text.splitlines():
    key,value=line.split('=',1); assert key in expected and key not in values and value and '=' not in value
    values[key]=value
...
assert values=={'schema':'1','run_id':run,'verdict':'VERIFIED_AWAITING_STATUS',
                'evidence_root_device_inode':evidence,'retry_manifest_sha256':manifest,
