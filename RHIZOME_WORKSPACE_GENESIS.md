...
The command's storage shard must equal the shard identity installed from the
process-lifetime `ShardProcessGuard` into that same coordinator. Genesis does
not own a separate configured shard value. The identical guarded value is later
bound into NBD Install authority, so a side channel cannot make materialization
and connection admission disagree about the local writer.
